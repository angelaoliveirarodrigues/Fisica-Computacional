...
/*
const vector<ODEpoint>& ODEsolver::PredictorCorrector(ODEpoint i, double step,double T){
    vector<ODEpoint> V2; //vetor para armazenas os pontos de solução
    V2.push_back(i); //inicializar o vetor com o ponto i

    while(V2.back().T()<T){ //Enquanto o tempo for menor que o tempo total
        ODEpoint Pcurrent= V2.back(); //Vamos utilizar o último ponto de V2 como o ponto atual
        vector<double> novoX(Pcurrent.X().size()); // vetor para armazenar os novos valores da componente Pcurrent.X()

        for (int n = 0; n < Pcurrent.X().size(); n++) {   //loop nas componente do vetor Pcurrent.X()
            ODEpoint Pnext(V2.back().T() + step, {Pcurrent.X()[n] + step * F()[n](Pcurrent)}); //por euler calcular o ponto "previsto"
            novoX[n]=Pcurrent.X()[n] +0.5 * step * (F()[n](Pcurrent) + F()[n](Pnext)); //calcular o valor com a fórmula do predictor
        }
        V2.push_back(ODEpoint(V2.back().T() + step, novoX)); // adicionar a solução a V2 para cada componente n; 
        //O tempo do novo ponto é calculado adicionando o step ao tempo do último ponto em V2. O vetor novoX contem os novos valores das componente, pelo que é usado para construir o novo ponto ODEpoint, que é adicionado a V2.
    }

    //Armazenar as soluções
    MS.insert({"trapezoidal",V2});
    ...
    while(V.back().T() < T){
        ...
        for (int n = 0; n < Pcurrent.X().size(); n++){
           double K1 = step * F()[n](Pcurrent);
            double K2 = step * F()[n](ODEpoint(Pcurrent.T() + step, Pcurrent.X()[n] + K1));
            novoX[n] = Pcurrent.X()[n] + (K1 + K2) / 2.0;
        }
        V.push_back(ODEpoint(V.back().T() + step, novoX));
    }
