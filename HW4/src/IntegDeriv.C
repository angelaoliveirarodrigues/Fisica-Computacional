...
void IntegDeriv::TrapezoidalRule(double xi, double xf, double& Integral, double& Error){
    //para apenas 1 intervalo
/*
    double h=(xf-xi); // calcular a largura do intervalo
    double yi= F(xi);
    double yf= F(xf);
    Integral= (h/2)*(yi+yf);
    double secondDerivative = IntegDeriv.fivepoint(xi, xf);
...
void IntegDeriv::simpsonRule(double xi, double xf, double& Integral, double& Error){
    ...
    double xm = (xf+xi)/2;
    double ym= F(xm);
    Integral= h/3 *(yi+4*ym+yf);
    double fourthDerivative=IntegDeriv.fivePointDiff(xi,xf, 4);
